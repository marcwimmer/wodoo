        ...
        for ext in set(str(x) for x in extensions):
            ...
            if not channels:
                def _filter(channel_name):
                    regexes = []
                    regexes.append(r'.+\/{}$'.format(ext))
                    regexes.append(r'.+\/{}-'.format(ext))
                    for regex in regexes:
                        if re.findall(regex, channel_name['name']):
                            return True
                    return False
    ...
    try:
        ...
        if msg.topic.startswith("asterisk/Console/result/"):
            ...
            if payload.get('odoo_instance'):
                ...
                id = int(id)
                connector._odoo('asterisk.connector', 'on_channel_originated', model, [id], channel_id, extension)
