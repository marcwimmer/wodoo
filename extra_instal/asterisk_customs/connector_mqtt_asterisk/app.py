...
class Asterisk_ACM(MQTT_Endpoint, EventListener):
    ...
    def run_Console(self, cmd, id=None):
        if os.getenv('ASTERISK_SSH_PORT', ""):
    ...
    def run_AMI(self, cmd, id=None):
        ...
        client = AMIClient(address=os.environ['ASTERISK_SERVER'], port=int(os.getenv('ASTERISK_AMI_PORT', '5038')))
    ...
    def on_message(self, client, userdata, message):
        try:
            ...
            if message.topic.startswith("asterisk/Console/send"):
                ...
                logger.debug("Originate with: {}".format(params))
    ...
    def connect_amiclient(self):
        ...
        amiclient = AMIClient(address=os.environ['ASTERISK_SERVER'], port=int(os.getenv('ASTERISK_AMI_PORT', '5038')))
        username = os.environ["ASTERISK_AMI_USER"]
        logger.debug('logging in {}'.format(username))
    ...
    def connect_ariclient(self):
        ...
        url = "ws://{host}:{port}{asterisk_prefix}/ari/events?api_key={user}:{password}&app={app}".format(
            ...
            app=ARI_APP_NAME,
            asterisk_prefix=os.environ["ASTERISK_PREFIX"],
        )
        while True:
            try:
        ...
        ariclient.on_bridge_event("BridgeCreated", self.onBridgeCreated)
        ariclient.on_bridge_event("BridgeDestroyed", self.onBridgeDestroyed)
        ariclient.on_bridge_event("BridgeMerged", self.onBridgeMerged)
        self.ariclient = ariclient

    def onBridgeAttendedTransfer(self, legs, ev, *args, **kwargs):
        logger.debug("AttendedTransfer done")
        self.publish("asterisk/ari/attended_transfer_done", json.dumps({
            'event': ev,
        }))

    def onBridgeCreated(self, bridge, *args, **kwargs):
        pass

    def onBridgeDestroyed(self, bridge, *args, **kwargs):
        pass

    def onBridgeMerged(self, bridge, bridge_from, *args, **kwargs):
        pass

    def onBridgeBlindTransfer(self, bridge, ev):
        logger.debug("Blind Transfer")
        self.publish("asterisk/ari/blind_transfer", json.dumps({
            'channel_ids': ev['bridge']['channels'],
        }))

    def onBridgeEntered(self, channel, ev):
        logger.debug("Bridge entered {}".format(channel.id))
        bridge = ev['bridge']
    ...
    def onBridgeLeft(self, channel, ev):
        logger.debug("Bridge left {}".format(channel.id))
        bridge = ev['bridge']
    ...
    def onChannelDestroyed(self, channel_obj, ev):
        logger.debug("Channel destroyed: {}".format(channel_obj))
        channel = channel_obj.json
        channel['state'] = "Down"
        self.on_channel_change(channel)
        self.on_channel_destroyed(channel)
    ...
    def on_channel_change(self, channel_json):
        logger.debug("Channel changed: {} to {}".format(channel_json['id'], channel_json['state']))
        self.publish("asterisk/ari/channel_update", json.dumps(channel_json))

    def on_channel_destroyed(self, channel_json):
        self.publish("asterisk/ari/channel_destroyed", json.dumps(channel_json))
