...
#        self.publish("asterisk/extension/pickupgroup/result",p.strip())
    ...
    def create_extension(self,data):
        print(data)
        try:
            self.pbxdb.remove_user(data["extension"])
            logger.debug("Previous User %r, removed.",data["extension"])
        except:
            pass
        ...
        self.pbxdb.add_user(
            ...
            outboundcid=data.get("outboundcid","4989416148870 <4989416148870>"),
            name=data.get("displayname",data["extension"]))

        self.pbxdb.add_extension(account=data["extension"], secret=data["secret"])

        self.apply_db_changes()

    def update_extension(self,data):
        self.pbxdb.update_extension(
            extension=data["extension"],
    ...
    def remove_extension(self,data):
        self.pbxdb.remove_extension(data["extension"])
        self.pbxdb.remove_user(data["extension"])
        self.apply_db_changes()
    ...
    def run(self):
        ...
        self.mqttclient.subscribe("asterisk/man_extension")
...
class FPBXDBConnector(object):
    ...
    def add_user(self,**params):
        ...
        qry = sqlalchemy.select([table.c.id]).order_by(table.c.id.desc())
        last_id=int(self.conn.execute(qry).fetchone()[0])
        ...
        usrman_values["username"]="UN-{}".format(params["extension"])
    ...
    def remove_user(self,extension_id):
        extension_id=int(extension_id)
        table=self.metadata.tables["devices"]
        self.conn.execute(table.delete().where(table.c.id == extension_id))
        table = self.metadata.tables["userman_users"]
        qry = sqlalchemy.select([table.c.id,table.c.username]).where(table.c.username==extension_id)
        id = self.conn.execute(qry).fetchone()
        if id:
            print(id)
            self.conn.execute(table.delete().where(table.c.id == id))
        table = self.metadata.tables["users"]
        self.conn.execute(table.delete().where(table.c.extension == extension_id))
    ...
    def update_extension(self,**params):
        table = self.metadata.tables["sip"]
        qry=table.update().where(table.c.id == params["extension"]).values(account=params["new_extension"], password=params["secret"])
    ...
    def remove_extension(self,extension_id):
        table=self.metadata.tables["sip"]
        self.conn.execute(table.delete().where(table.c.id == extension_id))
        self.show_extension(extension_id)
