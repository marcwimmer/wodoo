...
def _prepare_docker_compose_files(config, dest_file, paths):
    ...
    merged = '\n'.join(map(lambda x: x[1], final_contents))
    temp_filename = os.path.join(local_odoo_home, 'run', 'tmp.yml')
    with open(temp_filename, 'w', 0) as f:
        f.write(merged)
        f.flush()
    cmdline = []
    cmdline.append("/usr/local/bin/docker-compose")
    cmdline.append("-f")
    cmdline.append(temp_filename)
