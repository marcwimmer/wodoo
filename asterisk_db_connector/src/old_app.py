...
class Asterisk_ACM(EventListener):
    ...
    def set_pickup_group(self, extension, pickup_groups):
        from .app import FPBXDBConnector
        pbxdb = FPBXDBConnector()
        logger.debug("connecting to asterisk internal db")
        pbxdb.connect()
        logger.debug("setting pickupgroup %r for extension %r in database", pickup_groups, extension)
        pbxdb.update_pickupgroup(extension, pickup_groups)
        logger.debug("disconnected from asterisk internal db")
    ...
    def on_message(self, client, userdata, message):
        ...
        try:
            ...
            elif message.topic == 'asterisk/extension/pickupgroup':
                params = json.loads(message.payload)
                logger.info(params)
                extension = params["extension"]
                pickup_groups = params["pickupgroups"]
                self.set_pg(extension, pickup_groups)
    ...
    def connect_ariclient(self):
        ...
        ariclient.applications.subscribe(applicationName=[ARI_APP_NAME],
                                         eventSource="endpoint:SIP,bridge:")  # or just endpoint:
